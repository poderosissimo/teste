/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { ManutencaoWhereInput } from "./ManutencaoWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { ManutencaoOrderByInput } from "./ManutencaoOrderByInput";

@ArgsType()
class ManutencaoFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => ManutencaoWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => ManutencaoWhereInput, { nullable: true })
  @Type(() => ManutencaoWhereInput)
  where?: ManutencaoWhereInput;

  @ApiProperty({
    required: false,
    type: [ManutencaoOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [ManutencaoOrderByInput], { nullable: true })
  @Type(() => ManutencaoOrderByInput)
  orderBy?: Array<ManutencaoOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { ManutencaoFindManyArgs as ManutencaoFindManyArgs };
