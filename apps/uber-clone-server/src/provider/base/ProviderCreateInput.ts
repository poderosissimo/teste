/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import {
  IsString,
  IsOptional,
  IsNumber,
  ValidateNested,
} from "class-validator";
import { IsJSONValue } from "../../validators";
import { GraphQLJSON } from "graphql-type-json";
import { InputJsonValue } from "../../types";
import { ReviewCreateNestedManyWithoutProvidersInput } from "./ReviewCreateNestedManyWithoutProvidersInput";
import { Type } from "class-transformer";
import { ServiceBookingCreateNestedManyWithoutProvidersInput } from "./ServiceBookingCreateNestedManyWithoutProvidersInput";
import { SupportTicketCreateNestedManyWithoutProvidersInput } from "./SupportTicketCreateNestedManyWithoutProvidersInput";

@InputType()
class ProviderCreateInput {
  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  email?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  phoneNumber?: string | null;

  @ApiProperty({
    required: false,
  })
  @IsJSONValue()
  @IsOptional()
  @Field(() => GraphQLJSON, {
    nullable: true,
  })
  profilePicture?: InputJsonValue;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsNumber()
  @IsOptional()
  @Field(() => Number, {
    nullable: true,
  })
  rating?: number | null;

  @ApiProperty({
    required: false,
    type: () => ReviewCreateNestedManyWithoutProvidersInput,
  })
  @ValidateNested()
  @Type(() => ReviewCreateNestedManyWithoutProvidersInput)
  @IsOptional()
  @Field(() => ReviewCreateNestedManyWithoutProvidersInput, {
    nullable: true,
  })
  reviews?: ReviewCreateNestedManyWithoutProvidersInput;

  @ApiProperty({
    required: false,
    type: () => ServiceBookingCreateNestedManyWithoutProvidersInput,
  })
  @ValidateNested()
  @Type(() => ServiceBookingCreateNestedManyWithoutProvidersInput)
  @IsOptional()
  @Field(() => ServiceBookingCreateNestedManyWithoutProvidersInput, {
    nullable: true,
  })
  serviceBookings?: ServiceBookingCreateNestedManyWithoutProvidersInput;

  @ApiProperty({
    required: false,
    type: () => SupportTicketCreateNestedManyWithoutProvidersInput,
  })
  @ValidateNested()
  @Type(() => SupportTicketCreateNestedManyWithoutProvidersInput)
  @IsOptional()
  @Field(() => SupportTicketCreateNestedManyWithoutProvidersInput, {
    nullable: true,
  })
  supportTickets?: SupportTicketCreateNestedManyWithoutProvidersInput;
}

export { ProviderCreateInput as ProviderCreateInput };
